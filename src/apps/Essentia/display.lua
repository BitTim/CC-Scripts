...
function Module:new(x, y, aspect, title, color)
    ...
    local titleLabel = uilib.Label:new(title, 2, 2, uilib.Style:new(color, colors.black))
    mod.ui:add(titleLabel, "titleLabel")

    local amountLabel = uilib.Label:new("x" .. mod.amount, 3, 3, uilib.Style:new(colors.white, colors.black))
    mod.ui:add(amountLabel, "amountLabel")

    local progbar = uilib.ProgressBar:new(0, 250, mod.amount, 2, 5, 15, 1, false, false, uilib.Style:new(color))
    mod.ui:add(progbar, "progbar")

    local statusLabel = uilib.Label:new("Closed", 2, 7, closedLabelStyle)
    mod.ui:add(statusLabel, "statusLabel")

    local runBtn = uilib.Button:new(mod.selAmount, 5, 9, 9, 3, mod:run(), nil, false)
    mod.ui:add(runBtn, "runBtn")
    
    local lessBtn = uilib.Button:new("<", 2, 9, 3, 3, mod:less(), nil, false)
    mod.ui:add(lessBtn, "lessBtn")

    local moreBtn = uilib.Button:new(">", 14, 9, 3, 3, mod:more(), nil, false)
    mod.ui:add(moreBtn, "moreBtn")

    return mod
end

function Module:update(amount, status)
    self.amount = amount
    self.status = status
...
function Module:less()
    self.selAmount = self.selAmount - 5
    if self.selAmount < 0 then self.selAmount = 0 end

    if self.selAmount > self.amount then self.ui:get("runBtn").disabled = true
    else self.ui:get("runBtn").disabled = false end
end

function Module:more()
    self.selAmount = self.selAmount + 5
    if self.selAmount > 250 then self.selAmount = 250 end
...
local function createModule(page, x, y, aspect, title, color)
    ...
    if pages:get(page) == nil then
        ...
        pageGroup:add(mod, aspect)

        pages:add(pageGroup, page)
    else
        pages:get(page):add(mod, aspect)
    end
